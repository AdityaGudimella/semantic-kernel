...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            _WeaviateRecord(
                id_="1",
                description="A classic pangram.",
                text="The quick brown fox jumps over the lazy dog.",
                additional_metadata="additional info",
                is_reference=False,
                vector=[0.1, 0.1],
            ),
        ),
        (
            MemoryRecord(
                ...
                description="Another popular pangram.",
                additional_metadata="additional info",
                embedding=np.array([0.1, 0.11]),
            ),
            _WeaviateRecord(
                id_="2",
                description="Another popular pangram.",
...
class TestORMConversion:
    """I should be able to convert `MemoryRecord`` to `_WeaviateRecord` and back."""

    @staticmethod
    def _check_records_equal(act: pdt.BaseModel, exp: pdt.BaseModel) -> None:
        """Check that two records are equal."""
        act_dict = act.dict()
        exp_dict = exp.dict()
        embedding_key = "vector" if isinstance(act, _WeaviateRecord) else "embedding"
        act_vector = act_dict.pop(embedding_key)
        exp_vector = exp_dict.pop(embedding_key)
        assert act_dict == exp_dict
        assert act_vector == pytest.approx(exp_vector)

    def test_conversion_to_weaviate_record(
        self, memory_record: MemoryRecord, weaviate_record: _WeaviateRecord
    ) -> None:
        """I can create weaviate records from MemoryRecord."""
        act = _WeaviateRecord.from_orm(memory_record)
        self._check_records_equal(act, weaviate_record)

    def test_conversion_to_memory_record(
        self, memory_record: MemoryRecord, weaviate_record: _WeaviateRecord
    ) -> None:
        """I can create MemoryRecord from weaviate records."""
        act = weaviate_record.to_memory_record()
        self._check_records_equal(act, memory_record)
...
@pytest.mark.skipif(
    ...
    reason="Embedded weaviate only runs on Linux at the moment",
)
class TestEmbeddedWeaviate:
    def test_embedded_weaviate(self):
        config = weaviate_memory_store.WeaviateConfig(use_embed=True)
    ...
    @pytest.mark.asyncio
    async def test_get_collections(self, memory_store):
    ...
    @pytest.mark.asyncio
    async def test_delete_collection(self, memory_store_with_empty_collection):
    ...
    @pytest.mark.asyncio
    async def test_collection_exists(self, memory_store_with_empty_collection):
        collection_name, memory_store = memory_store_with_empty_collection
    ...
    @pytest.mark.asyncio
    async def test_upsert(self, memory_store_with_empty_collection, documents):
        collection_name, memory_store = memory_store_with_empty_collection
    ...
    @pytest.mark.asyncio
    async def test_upsert_batch(self, memory_store_with_empty_collection, documents):
        collection_name, memory_store = memory_store_with_empty_collection
    ...
    @pytest.mark.asyncio
    async def test_get(self, memory_store_with_collection, documents):
    ...
    @pytest.mark.asyncio
    async def test_get_batch(self, memory_store_with_collection, documents):
        collection_name, memory_store = memory_store_with_collection
    ...
    @pytest.mark.asyncio
    async def test_remove_batch(self, memory_store_with_collection, documents):
        collection_name, memory_store = memory_store_with_collection
        ...
        remaining_docs = memory_store.client.data_object.get(
            class_name=collection_name
        )["totalResults"]
        assert remaining_docs == len(documents) - len(keys)

    @pytest.mark.asyncio
    async def test_remove(self, memory_store_with_collection, documents):
        collection_name, memory_store = memory_store_with_collection
    ...
    @pytest.mark.asyncio
    async def test_get_nearest_matches(self, memory_store_with_collection, documents):
        collection_name, memory_store = memory_store_with_collection
    ...
    @pytest.mark.asyncio
    async def test_get_nearest_match(self, memory_store_with_collection, documents):
        collection_name, memory_store = memory_store_with_collection
